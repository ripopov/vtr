...
fn css_color<E: serde::de::Error>(value: Option<String>) -> Result<Option<Color>, E> {
    value
        .map(|s| {
...
fn optional_color<'de, D: Deserializer<'de>>(d: D) -> Result<Option<Color>, D::Error> {
    css_color(Option::<String>::deserialize(d)?)
}

fn chart_colors<'de, D: Deserializer<'de>>(d: D) -> Result<[Option<Color>; 6], D::Error> {
