...
crates/volna           GPUI frontend: native macOS app, wasm page, VS Code extension
...
crates/volna-egui      egui/eframe frontend: native desktop only
...
close or session replacement makes late results no-ops. This is the same
ownership model the viewer had before the split, now testable with a plain
loop that performs requests in any order.
...
at 1× and 2× DPI. The display-list indirection costs nothing measurable: a
frame is a few thousand primitives in a reused buffer, and the shaped-text cache
the split introduced made frames faster than before (see `VERIFICATION.md`).
...
again, updating all its rows together. The VTR reader, file format and decode
path are unchanged.
