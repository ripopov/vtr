...
pub fn parse_css_color(text: &str) -> Option<Hsla> {
    let text = text.trim();
    ...
    Some(
        Rgba {
            r: number(parts[0], 255.0)?,
            g: number(parts[1], 255.0)?,
            b: number(parts[2], 255.0)?,
            a: parts.get(3).map_or(Some(1.0), |a| number(a, 1.0))?,
        }
        .into(),
    )
}
