...
use vtr::*;
use vtr_kdb::{Database, Debugger, TraceNode};
...
fn trace(path: &std::path::Path, bad_width: bool, missing: bool) {
...
fn mux_reset_depth_and_stdout() {
    ...
    let out = Command::new(env!("CARGO_BIN_EXE_vtr-kdb"))
        .args([
            "trace",
            concat!(
...
fn procedural_order_arithmetic_sync_reset_and_nba_last_write() {
    ...
    let db = Database::open(concat!(
        env!("CARGO_MANIFEST_DIR"),
        "/tests/fixtures/semantics.kdb.json"
    ))
    .unwrap();
    let r = Reader::open(&path).unwrap();
    let mut d = Debugger::attach(&db, &r, "").unwrap();
...
fn dump_gaps_and_design_identity_are_checked() {
    ...
    w.set_file_attr("design.kdb_id", Value::Str(id)).unwrap();
    w.begin_scope("top", ScopeType::Module, "top");
    w.add_bits("a", 8, 4);
    w.end_scope().unwrap();
    w.close().unwrap();
    ...
    assert!(Debugger::attach(&db, &r, "")
        .err()
        .unwrap()
        .contains("design.kdb_id differs"));
}
