...
use vtr::*;
use vtr_vdb::{netlist::NetlistIndex, Database, Debugger};
fn fixture(name: &str) -> Database {
    Database::open(
        Path::new(env!("CARGO_MANIFEST_DIR")).join(format!("tests/fixtures/{name}.vdb.json")),
    )
    .unwrap()
}
...
fn validate_geometry(view: &vtr_vdb::netlist::LaidOutNetlist) {
...
fn cli_writes_svg_and_requires_time_output() {
    ...
    let base = || {
        let mut cmd = std::process::Command::new(env!("CARGO_BIN_EXE_vtr-vdb"));
        cmd.args([
            "netlist",
            concat!(
...
#[test]
fn old_vdb_versions_require_reexport() {
    let dir = tempfile::tempdir().unwrap();
    let file = dir.path().join("old.json");
    std::fs::write(&file, r#"{"format":"vtr-rtl-vdb","version":1}"#).unwrap();
