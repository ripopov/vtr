...
pub fn db() -> Database {
    ...
    if let Ok(python) = std::env::var("VTR_VDB_PYTHON") {
        ...
        let out = Command::new(python)
            .current_dir(&root)
            .args(["tools/vdb/export.py", "--top", "top", "-o"])
            .arg(&path)
            .arg("crates/vtr-vdb/tests/rtl/pipeline.sv")
            .output()
            .unwrap();
        assert!(
            out.status.success(),
        ...
        Database::open(root.join("crates/vtr-vdb/tests/fixtures/pipeline.vdb.json")).unwrap()
    }
}
