...
def build_log():
    """Builds the log-writer comparison (bench/log): VTR, NanoLog, binlog, Quill, CLP IR, text baseline."""
    if not os.path.exists(os.path.join(ROOT, "ext", "quill", "include")):
        sh(["sh", os.path.join(ROOT, "bench", "log", "fetch_refs.sh")])
    os.makedirs(LOG_BUILD, exist_ok=True)
    sh(["cmake", os.path.join(ROOT, "bench", "log"), "-DCMAKE_BUILD_TYPE=Release"], cwd=LOG_BUILD)
    sh(["make", "-j8"], cwd=LOG_BUILD)
...
LOG_WORKLOADS = {
    "sim_log_1m": ("1000000", "synthetic SoC simulation log: 1M timestamped messages from 13 call sites (bus transactions with hex addresses, per-core fetch/retire, cache events, warnings, DMA completions, parity errors, UART text, scheduler stats, interrupts, checkpoints, FSM transitions; 50% DEBUG / 46% INFO / 3% WARN / 1% ERROR), generated by bench/log/workload.hpp"),
    "sim_log_10m": ("10000000", "the same generator, 10M messages"),
}
SMALL_LOG = ["sim_log_1m"]
...
def timed(cmd, stdout=subprocess.DEVNULL):
    """Wall time of an external command whose output is discarded (decoders of other loggers)."""
    t = time.time()
    print("+", " ".join(cmd), flush=True)
    subprocess.run(cmd, stdout=stdout, stderr=subprocess.DEVNULL, check=True)
    return time.time() - t


def run_log(name, n, repeat, out_dir):
    """Writes the same message stream with every logger, then reads each file back to text."""
    b = lambda x: os.path.join(LOG_BUILD, x)
    p = lambda ext: os.path.join(out_dir, f"{name}.{ext}")
    res = {"workload": name, "n": int(n), "info": {"description": LOG_WORKLOADS[name][1]}, "machine": machine_info(), "writers": {}, "decode": {}, "check": {}}
    writers = [
        ("text", [b("log_text"), n, p("log")]),
        ("quill", [b("log_quill"), n, p("quill.log")]),
        ("binlog", [b("log_binlog"), n, p("blog")]),
        ("nanolog", [b("log_nanolog"), n, p("nlog")]),
        ("clp-ir", [b("log_clp"), n, p("clp.zst")]),
        ("vtr", [b("log_vtr"), n, p("vtr")]),
        ("vtr-inline", [b("log_vtr"), n, p("inline.vtr"), "--inline"]),
    ]
    for label, cmd in writers:
        res["writers"][label] = best_of_key(repeat, lambda: json_out(cmd), "total_s")
        r = res["writers"][label]
        print(f"  {name} {label:11s} loop {r['loop_s']:.3f}s total {r['total_s']:.3f}s cpu {r['cpu_s']:.3f}s {r['bytes']:>12} bytes", flush=True)
    for label, path in (("text", p("log")), ("quill", p("quill.log")), ("binlog", p("blog"))):
        res["writers"][label]["zstd"] = json_out([b("log_zstd"), path])
    # Read back to text.
    res["decode"]["vtr"] = best_of_key(repeat, lambda: json_out([b("log_vtr"), "--decode", p("vtr")]), "decode_s")
    res["decode"]["clp-ir"] = best_of_key(repeat, lambda: json_out([b("log_clp"), "--decode", p("clp.zst")]), "decode_s")
    res["decode"]["nanolog"] = {"decode_s": min(timed([b("nanolog_decompressor"), "decompress", p("nlog")]) for _ in range(repeat))}
    res["decode"]["binlog"] = {"decode_s": min(timed([b("bread"), p("blog")]) for _ in range(repeat))}
    res["query"] = {"vtr-warn": best_of_key(repeat, lambda: json_out([b("log_vtr"), "--warn", p("vtr")]), "decode_s")}
    # Information check: the text rendered from the VTR and CLP files must equal the text log byte for byte;
    # NanoLog's decompressor and bread print their own prefixes, so only their line counts are compared.
    text = open(p("log"), "rb").read()
    vtr_txt = subprocess.run([b("log_vtr"), "--decode", p("vtr"), "--print"], capture_output=True).stdout
    clp_txt = subprocess.run([b("log_clp"), "--decode", p("clp.zst"), "--print"], capture_output=True).stdout
    nl_lines = subprocess.run([b("nanolog_decompressor"), "decompress", p("nlog")], capture_output=True).stdout.count(b"\n")
    bl_lines = subprocess.run([b("bread"), p("blog")], capture_output=True).stdout.count(b"\n")
    res["check"] = {
        "text_lines": text.count(b"\n"),
        "vtr_text_identical": vtr_txt == text,
        "clp_text_identical": clp_txt == text,
        "nanolog_lines": nl_lines,
        "binlog_lines": bl_lines,
    }
    print(f"  check: {res['check']}", flush=True)
    # The outputs are large (670 MB of text at 10M messages) and fully described by the numbers: remove them.
    for label in ("log", "quill.log", "blog", "nlog", "clp.zst", "vtr", "inline.vtr"):
        try:
            os.remove(p(label))
        except OSError:
            pass
...
def summary(rtl, tx, logs=()):
    ...
    if logs:
        ...
        for r in logs:
            w = r["writers"]
            v = w["vtr"]
            others = {k: w[k] for k in ("nanolog", "binlog", "quill", "clp-ir", "text") if k in w}
            smallest = min(others, key=lambda k: others[k]["bytes"])
            fastest_loop = min(others, key=lambda k: others[k]["loop_s"])
            fastest_total = min(others, key=lambda k: others[k]["total_s"])
            mps = lambda x: r["n"] / x / 1e6
            parts.append(
                f"{r['workload']}: VTR file {v['bytes'] / others[smallest]['bytes'] * 100:.0f}% of the smallest other output ({smallest}) and "
                f"{v['bytes'] / w['text']['zstd']['bytes'] * 100:.0f}% of the zstd-compressed text log; "
                f"hot path {mps(v['loop_s']):.1f} M msg/s (fastest other: {fastest_loop} {mps(others[fastest_loop]['loop_s']):.1f}), "
                f"loop plus flush {mps(v['total_s']):.1f} M msg/s (fastest other: {fastest_total} {mps(others[fastest_total]['total_s']):.1f}); "
                f"read back to text {r['n'] / r['decode']['vtr']['decode_s'] / 1e6:.1f} M lines/s")
        L.append("- **Logs.** " + "; ".join(parts) + ".")
    L.append("")
    return L
...
def render(results, path):
    ...
    tx = results["tx"]
    logs = results.get("log", [])
    L.extend(summary(rtl, tx, logs))
    if rtl:
...
def render_logs(logs):
    ...
    L.append("Every logger receives the same messages (see `docs/BENCHMARKS.md`, *Log workloads*): one producer thread, simulation-time stamps, "
             "13 call sites with integer, hex, float and string arguments. `hot path` is the message loop alone (asynchronous loggers queue and return); "
             "`total` includes the flush/close that puts everything on disk; `cpu` is user+system time of the whole process, background threads included. "
             "`+zstd` is the size after compressing the output with zstd level 3 in 4 MiB frames, for the loggers that write uncompressed output "
             "(VTR, NanoLog and the CLP IR stream are already compressed). `read back` renders every record to text again "
             "(VTR: `vtr_log_rec_format`; CLP: IR decoder; NanoLog: its `decompressor`; binlog: `bread`; Quill and the text baseline already are text).\n")
    for r in logs:
        n = r["n"]
        w = r["writers"]
        m = r.get("machine", {})
        L.append(f"### {r['workload']} ({n:,} messages)\n")
        if m:
            L.append(f"Run on {m.get('cpu', '')} ({m.get('cores')} threads, {m.get('memory')}), {m.get('os', '')} {m.get('kernel', '')}, Apple clang / {m.get('rustc', '')}, {m.get('date', '')}.\n")
        L.append("| logger | output | hot path | M msg/s | total (loop + flush) | cpu | size | bytes / msg | +zstd | read back to text |")
        L.append("|---|---|---:|---:|---:|---:|---:|---:|---:|---:|")
        rows = [
            ("text (fprintf)", "text", "text", "plain text"),
            ("Quill", "quill", "quill", "text (async)"),
            ("binlog", "binlog", "binlog", "binary, uncompressed"),
            ("NanoLog", "nanolog", "nanolog", "binary, packed"),
            ("CLP IR + zstd", "clp-ir", "clp-ir", "logtype + variables, zstd"),
            ("**VTR**", "vtr", "vtr", "LOG blocks, zstd"),
            ("VTR inline (no background thread)", "vtr-inline", "vtr", "LOG blocks, zstd"),
        ]
        for label, key, dkey, what in rows:
            if key not in w:
                continue
            x = w[key]
            z = f"{fmt_bytes(x['zstd']['bytes'])} ({x['zstd']['zstd_s']:.2f}s)" if "zstd" in x else "-"
            d = r["decode"].get(dkey)
            if key == "vtr-inline":
                dec = "-"
            elif d is None:
                ...
                dec = f"{d['decode_s']:.3f}s ({n / d['decode_s'] / 1e6:.1f} M lines/s)"
            bold = key == "vtr"
            b = lambda s: f"**{s}**" if bold else s
            L.append(f"| {label} | {what} | {b(f'{x["loop_s"]:.3f}s')} | {n / x['loop_s'] / 1e6:.1f} | {b(f'{x["total_s"]:.3f}s')} | {x['cpu_s']:.2f}s | {b(fmt_bytes(x['bytes']))} | {x['bytes'] / n:.1f} | {z} | {dec} |")
        q = r.get("query", {}).get("vtr-warn")
        if q:
            L.append(f"\nVTR severity query: all records at WARN or above ({q['lines']:,} of {n:,}) selected in {q['decode_s'] * 1000:.1f} ms without decoding the other blocks' arguments.")
        c = r.get("check", {})
        if c:
            L.append(f"\nInformation check: text rendered from the VTR file identical to the text log: {'yes' if c.get('vtr_text_identical') else 'NO'}; "
                     f"from the CLP IR stream: {'yes' if c.get('clp_text_identical') else 'NO'}; NanoLog decompressor lines {c.get('nanolog_lines', 0):,} "
...
def main():
    ...
    if a.workloads:
        ...
        log_names = [n for n in log_names if n in sel]
    if a.what == "compilers":
        import compilers
        sys.argv = [sys.argv[0]]
        compilers.main()
