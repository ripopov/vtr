// Loader for transaction replay files (bench/vtr-bench/src/tx.rs, "VTRTXR02").
#pragma once
#include <cstdint>
#include <cstdio>
#include <cstring>
...
struct TxOp { uint8_t op; uint64_t id; uint32_t gen_or_key; uint8_t ty; uint64_t v; uint64_t time; uint32_t kind; uint64_t from, to; };
struct TxReplay {
...
static bool tx_load(const char *path, TxReplay &rp) {
    ...
    if (len < 8 || memcmp(d.data(), "VTRTXR02", 8) != 0) { fprintf(stderr, "not a tx replay\n"); return false; }
    size_t p = 8;
    ...
    for (size_t i = 0; i < nops; i++) {
        ...
        case 2: o.id = tx_varint(d.data(), p); o.gen_or_key = (uint32_t)tx_varint(d.data(), p); o.ty = d[p++];
