...
function renderRange(){const overlay=$('range-overlay');overlay.hidden=!state.range;if(state.range){overlay.style.left=LABEL+state.range[0]*state.zoom+'px';overlay.style.width=(state.range[1]-state.range[0])*state.zoom+'px';$('range-start').value=state.range[0];$('range-end').value=state.range[1];}$('clear-range').disabled=!state.range;$('measure-mode').setAttribute('aria-pressed',state.mode==='measure');$('pan-mode').setAttribute('aria-pressed',state.mode==='pan');$('pan-mode').classList.toggle('selected',state.mode==='pan');$('measure-mode').classList.toggle('selected',state.mode==='measure');}
...
function renderOverview(){
  const occupancy=Array.from({length:TOTAL},(_,c)=>data.filter(o=>o.start<=c&&o.end>c).length),max=Math.max(...occupancy,1);
  $('overview-svg').innerHTML=occupancy.map((n,c)=>`<rect x="${c/TOTAL*1000}" y="${47-n/max*41}" width="${1000/TOTAL-1.5}" height="${n/max*41+1}" rx="1" fill="${data.some(o=>o.miss&&o.start<=c&&o.ready>c)?'#c4a66e':'var(--accent-line)'}" opacity=".7"/>`).join('');
...
function renderOverviewWindow(){const sc=$('chart-scroll');const start=sc.scrollLeft/state.zoom, span=Math.max(0,sc.clientWidth-LABEL)/state.zoom;const w=$('overview-window');w.style.left=(start/TOTAL*100)+'%';w.style.width=(Math.min(TOTAL-start,span)/TOTAL*100)+'%';$('overview').setAttribute('aria-valuenow',Math.round(start));$('overview').setAttribute('aria-valuemax',Math.max(0,Math.floor(TOTAL-span)));$('overview').setAttribute('aria-valuetext',`Cycles ${Math.floor(start)} to ${Math.min(TOTAL,Math.ceil(start+span))}`);$('visible-range').textContent=`${Math.floor(start)}–${Math.min(TOTAL,Math.ceil(start+span))} cyc`;}
