...
EXPORT = ROOT / 'integrations/slang/export.py'
        ...
        for name in ('pipeline', 'semantics', 'netlist', 'edge_cases'):
            with self.subTest(name=name), tempfile.TemporaryDirectory() as td:
                ...
                p = subprocess.run([sys.executable, str(EXPORT), '--top', 'top', '-o', str(out),
                                    f'core/vtr-vdb/tests/rtl/{name}.sv'], capture_output=True, text=True, cwd=ROOT)
                self.assertEqual(p.returncode, 0, p.stderr)
                expected = json.loads((ROOT/f'core/vtr-vdb/tests/fixtures/{name}.vdb.json').read_text())
                self.assertEqual(json.loads(out.read_text()), expected)
                self.assertEqual([f['path'] for f in expected['source_index']['files']],
                                 [f'core/vtr-vdb/tests/rtl/{name}.sv'])
