# Application note: building a KDB on top of VTR

VTR stores what happened in a simulation. It deliberately stores nothing
about how to *show* it. A KDB (knowledge database) is the separate,
...
is defined. The same VTR file can be rendered by many KDBs, and a KDB
written for one design keeps working across simulation runs because it
attaches to VTR by *names*, never by ids.

This note explains the attachment contract, gives a complete worked KDB
...
assigned per file and a KDB must resolve names to ids at load time
(`Reader::find_node`, `find_signal`, or a walk over `Hierarchy::nodes`).

### 1.2 What a KDB is

A KDB is a document (JSON, YAML, SQLite, a Rust struct — VTR does not care)
...
Loading a KDB means evaluating every selector against the hierarchy and
the string table once, producing id-keyed tables the viewer uses. Nothing
is written back to the VTR file.

### 1.3 Reading pattern

A KDB-driven viewer opens the file (cheap: directory, strings, hierarchy),
...
3. Intern the attribute keys, stage names, lane names and relation kinds
   the KDB uses into string ids (`StringTable::find`, or build a
   `HashMap<&str, StrId>` over the table once). Missing names are not
   errors: a KDB may describe more stages than a particular run used.
...
as the KDB says; if a side has no such stage the arrow is not drawn.

*Labels and tooltips*: `label`/`detail` attributes and per-stage `label`
attributes, all by key id.

*Row identity across runs / A-B comparison*: the KDB names
...
| Kanata feature | VTR data | KDB semantic |
|---|---|---|
...
data and into the KDB, where they can differ per core generation.

### 2.6 Extending the viewer without touching the trace

* A second KDB for the same file can colour by `thread`, group rows by
...
* A SystemC/TLM trace (FTR converted, or written natively) uses the same
  viewer with a KDB whose `stages` map the `parent_of` relation into
...
attributes the simulator recorded, e.g. `fst.source_stem`). A debugger KDB
adds:

1. **Source links**: selectors on scope paths and var paths to
   `{file, line}`; when the simulator emitted `fst.source_stem`
   attributes the KDB can be generated from them, otherwise from the
...
3. **Formatting**: radix, signedness, enum table binding by var path
   (VTR carries `enum_table` node references from FST enum refs; the KDB
...
5. **Trigger/search**: named conditions (`posedge clk && req && !ack`)
   ...
   change lists; the KDB stores the expressions, VTR provides the data.

The KDB is keyed by var path, so it survives re-simulation and even
